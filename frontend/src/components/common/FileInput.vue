<template>
    <div class="input-file">
        <div :class="{'border-red-300': errorMessage}" class="flex flex-col items-center py-4 border rounded-md">
            <div v-if="selectedFileName != ''" class=" text-xs bg-black/10 p-2 flex items-center space-x-1 rounded-md">
                ...
                <button type="button" class="flex items-center justify-center w-4 h-4 bg-gray-400 rounded-full"
                    @click="removeFile()"><i class='bx bx-x'></i></button>
            </div>
            ...
            <input ref="inputFile" @change="fileChange" type="file" class="hidden" placeholder="Upload file">
        </div>
